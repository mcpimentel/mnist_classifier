...
from models import LeNet_standard, LeNet_dropout
...
def test(model):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            if (cnt % (args.log_interval*200) == 0):
                print(f"Validation step {cnt} of {len(test_loader)}")
            cnt+=1
            if args.cuda:
                data, target = data.cuda(), target.cuda()
            data, target = Variable(data), Variable(target)
...
def mcdropout_test(model):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            test_loss += F.nll_loss(F.log_softmax(output_mean, dim=1), target, size_average=False).item()  # sum up batch loss
...
def main():
    if socket.gethostname() == "dgx-r92-3.g42cloud.net": # slurm-node="dgx1-r91-1.g42cloud.net"?
    ...
    if args.mode == 0:
        ...
        ckpt_dropout = torch.load('checkpoint/LeNet_dropout60.pth.tar')
        model_dropout.load_state_dict(ckpt_dropout['state_dict'])
        mcdropout_test(model_dropout)

    else:
        print ('--mode argument is invalid \ntrain mode (0) or test mode (1)')
